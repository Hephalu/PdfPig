namespace UglyToad.PdfPig.Tests.Hephalu
{
    using System;
    ...
    public class InvoiceParseTests
    {
        [Fact]
        public void ReadTanaInvoice()
        {
            var filePath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Hephalu","Documents", "TanaNormal.pdf");
            var pdfScanner = GetScanner(filePath);
            Assert.True(true);
        }
...
endobj";
        ...
        private static PdfTokenScanner GetScanner(string filename)
        {
            var fileBytes = File.ReadAllBytes(filename);
            var inputBytes = new ByteArrayInputBytes(fileBytes);

            return new PdfTokenScanner(inputBytes, new TestObjectLocationProvider(),
                new TestFilterProvider(), NoOpEncryptionHandler.Instance);
        }
